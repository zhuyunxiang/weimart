                                ...
                                case MANY_TO_MANY:
                                    ...
                                    switch (strtoupper($opType)){
                                        ...
                                        if(is_array($mappingData)) {
                                            ...
                                            $tableName = substr($tableName, strlen($prefix), strlen($tableName)-strlen($prefix));
                                            $md = D($tableName);
                                            $ids   = array();
                                            foreach ($mappingData as $key => $value) {
                                                if (is_array($value)) {
                                                    $condition = $value;
                                                    $re = $md->where($condition)->find();
                                                    if(isset($re)){
                                                        $ids[] = $re[$model->getPk()];
                                                    }else{
                                                        $ids[] = $md->add($value);
                                                    ...
                                                    $condition = $mappingData;
                                                    $re = $md->where($condition)->find();
                                                    if(isset($re)){
                                                        $relationId = $re[$model->getPk()];
                                                    }else{
                                                        $relationId = $md->add($mappingData);
                                                    }
                                                    break;
                                                }
                                            }
                                        ...
                                        // $prefix = $this->tablePrefix;
                                        // $mappingRelationTable = substr($mappingRelationTable, strlen($prefix), strlen($mappingRelationTable)-strlen($prefix));
                                        // $delModel = M($mappingRelationTable);
                                        // $result = $delModel->where($mappingCondition)->delete();
                                        break;
                                    }
                                    break;
                            }
